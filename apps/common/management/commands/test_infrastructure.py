...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("🚀 Testing Jobraker Backend Infrastructure")
        )
        self.stdout.write("")
    ...
    def check_django_config(self):
        ...
        if (
            settings.SECRET_KEY
            and settings.SECRET_KEY != "django-insecure-change-me-in-production"
        ):
    ...
    def check_database(self):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"  ✅ PostgreSQL connected: {version[:50]}...")
                )
                ...
                try:
                    ...
                    if cursor.fetchone():
                        ...
                        self.stdout.write(
                            self.style.WARNING("  ⚠️ pgvector extension not found")
                        )
                        self.stdout.write(
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"  ⚠️ Could not check pgvector: {e}")
                    )
    ...
    def check_celery(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS("  ✅ Celery app imported successfully")
            )
            ...
            try:
                ...
                if stats:
                    self.stdout.write(
                        self.style.SUCCESS("  ✅ Celery workers are active")
                    )
                else:
                    self.stdout.write(self.style.WARNING("  ⚠️ No Celery workers found"))
                    self.stdout.write(
                        "     Start worker: celery -A jobraker worker -l info"
                    )
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠️ Could not connect to Celery broker: {e}")
                )
    ...
    def check_external_apis(self):
        ...
        if adzuna_id and adzuna_key:
            self.stdout.write(
                self.style.SUCCESS("  ✅ Adzuna API credentials configured")
            )
        else:
            self.stdout.write(
                self.style.WARNING("  ⚠️ Adzuna API credentials not configured")
            )
