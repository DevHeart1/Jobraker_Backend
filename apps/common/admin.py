...
from .models import VectorDocument, KnowledgeArticle

@admin.register(VectorDocument)
class VectorDocumentAdmin(admin.ModelAdmin):
    list_display = ('id', 'source_type', 'source_id', 'text_content_preview', 'created_at', 'updated_at')
    list_filter = ('source_type', 'created_at')
    search_fields = ('source_id', 'text_content', 'metadata')
    readonly_fields = ('created_at', 'updated_at', 'embedding') # Embedding is too large to display well
...
class KnowledgeArticleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('title', 'slug', 'source_type', 'category', 'tags')
        }),
        ('Content', {
            'fields': ('content',)
        }),
