...
from pgvector.django import L2Distance, CosineDistance, InnerProduct
...
class VectorStorageService:
    ...
    def __init__(self):
        ...
        self.is_production = not settings.DEBUG and hasattr(VectorDocument._meta.get_field('embedding'), 'dimensions')
    ...
    def _process_batch(self, batch: List[Dict[str, Any]], update_existing: bool) -> Dict[str, int]:
        ...
        for doc in batch:
            try:
                ...
                logger.error(f"Error processing document {doc.get('source_id', 'unknown')}: {e}")
                stats['errors'] += 1
    ...
    def _store_single_document(self, doc: Dict[str, Any], update_existing: bool) -> str:
        ...
        if existing:
            if update_existing:
                ...
                existing.save(update_fields=['text_content', 'embedding', 'metadata', 'updated_at'])
                return 'updated'
            else:
                return 'skipped'
        else:
    ...
    def search_similar_vectors(
        ...
        try:
            ...
            if self.is_production:
                ...
                for doc in all_docs:
                    similarity = self._compute_cosine_similarity(query_embedding, doc.embedding)
                    if similarity >= similarity_threshold:
                        doc.similarity = similarity
                        scored_docs.append(doc)
                
                # Sort by similarity and take top_k
                similar_docs = sorted(scored_docs, key=lambda x: x.similarity, reverse=True)[:top_k]
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            source_types = self.model.objects.values_list('source_type', flat=True).distinct()
    ...
    def cleanup_old_vectors(self, days_old: int = 30, source_types: Optional[List[str]] = None) -> int:
        """
        ...
        try:
            ...
            logger.info(f"Cleaned up {deleted_count} old vector documents (older than {days_old} days)")
    ...
    def reindex_vectors(self) -> bool:
        ...
        try:
            with connection.cursor() as cursor:
                # Reindex HNSW index
                cursor.execute("REINDEX INDEX CONCURRENTLY vector_document_embedding_hnsw_idx;")
