...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_chat_message(self, data):
        ...
        get_openai_chat_response_task.delay(
            user_id=str(user.id), session_id=self.session_id, message=message_content
        )
    ...
    def save_message(self, user, content, sender_type):
        ...
        try:
            ...
            if user not in session.participants.all():
                logger.warning(
                    f"User {user.id} not in session {self.session_id}. Adding."
                )
                session.participants.add(user)

            message = ChatMessage.objects.create(
                session=session, sender=user, content=content, sender_type=sender_type
            )
            return message
