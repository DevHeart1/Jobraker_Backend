...
class ChatModelTests(APITestCase):
    ...
    def test_create_chat_session(self):
        ...
        self.assertEqual(
            str(session), f"Session {session.id} by {self.user.email} - 'Test Session'"
        )

    def test_create_chat_message(self):
        session = ChatSession.objects.create(
            user=self.user, title="Message Test Session"
        )
        message = ChatMessage.objects.create(
            session=session, role="user", content="Hello, world!"
        )
        ...
        self.assertEqual(
            str(message),
            f"User message in Session {session.id} at {message.timestamp.strftime('%Y-%m-%d %H:%M')}",
        )
...
class ChatSerializerTests(APITestCase):
    def setUp(self):
        ...
        self.session = ChatSession.objects.create(
            user=self.user, title="Serializer Test Session"
        )
        self.message = ChatMessage.objects.create(
            session=self.session, role="user", content="Test message content."
        )
    ...
    def test_chat_session_serializer(self):
        ...
        self.assertEqual(
            str(data["user"]), str(self.user.id)
        )  # User is serialized as UUID
...
class ChatAPITests(APITestCase):
    ...
    def test_list_chat_sessions_api(self):
        ...
        )  # Should only list sessions for self.user
    ...
    def test_send_message_new_session_api(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            ChatMessage.objects.count(), 2
        )  # User message + AI simulated message
        self.assertEqual(ChatSession.objects.count(), 1)
    ...
    def test_send_message_existing_session_api(self):
        self.client.force_authenticate(user=self.user)
        session = ChatSession.objects.create(
            user=self.user, title="Existing Session Test"
        )

        url = reverse("chat")
        data = {
            "session_id": session.id,
            "message_text": "Another message for existing session.",
        }
        response = self.client.post(url, data, format="json")
        ...
        self.assertEqual(
            response.data["user_message"]["content"],
            "Another message for existing session.",
        )
    ...
    def test_send_message_to_nonexistent_session_api(self):
        ...
        data = {
            "session_id": 9999,
            "message_text": "Message to nowhere.",
    ...
    def test_send_message_to_other_user_session_api(self):
        self.client.force_authenticate(user=self.user)
        other_user = User.objects.create_user(
            email="anotherapiuser@example.com",
            password="password",
            first_name="Another",
            last_name="User",
        )
        other_session = ChatSession.objects.create(
            user=other_user, title="Not My Session"
        )
        ...
        )  # Should be treated as not found for this user

    @patch("apps.integrations.services.openai_service.OpenAIJobAssistant.chat_response")
    def test_send_message_async_task_queued_successfully(
        self, mock_assistant_chat_response
    ):
        ...
        self.assertEqual(
            ChatMessage.objects.filter(role="assistant").count(), 0
        )  # AI message not saved yet
        ...
        self.assertEqual(
            response.data["user_message"]["content"],
            "Hello AI, process this asynchronously.",
        )
...
class ChatCeleryTaskTests(APITestCase):
    def setUp(self):
        ...
        self.session = ChatSession.objects.create(
            user=self.user, title="Celery Task Test Session"
        )
        # Create an initial user message in the session for history context
        ChatMessage.objects.create(
            session=self.session,
            role="user",
            content="Initial user message for history.",
        )
    ...
    def test_get_openai_chat_response_task_success(self, mock_openai_client):
        ...
        mock_client_instance.chat.completions.create.return_value = MagicMock(
            choices=[
                MagicMock(message=MagicMock(content="Mocked AI response from task."))
            ]
        )
        ...
        prepared_history = [
            {"role": "user", "content": "Initial user message for history."}
        ]
    ...
    @patch("openai.OpenAI")
    def test_get_openai_chat_response_task_with_conversation_history(
        ...
        mock_client_instance.chat.completions.create.return_value = MagicMock(
            choices=[
                MagicMock(
                    message=MagicMock(content="AI response with history context.")
                )
            ]
        )
