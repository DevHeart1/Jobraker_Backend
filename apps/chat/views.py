...
from .serializers import (ChatMessageSerializer, ChatResponseSerializer,
                          ...
                          ChatSessionListSerializer, ChatSessionSerializer,
...
class ChatView(APIView):
    ...
    def post(self, request):
        ...
        if data.get("session_id"):
            ...
            session = ChatSession.objects.create(
                user=user, title="New Chat", last_message_at=timezone.now()
            )

        user_msg = ChatMessage.objects.create(
            session=session, role="user", content=data["message_text"]
        )
        ...
        if result.get("status") == "queued":
            ...
            if (
                ...
                and "queue" in result.get("message", "").lower()
            ):
...
class JobAdviceView(APIView):
    ...
    def post(self, request):
        return Response(
            {"detail": "This feature (Job Advice) is not yet implemented."}, status=501
        )
    ...
    return Response(
        {
            ...
            "expires_in": access_token.payload.get("exp")
            - access_token.payload.get("iat"),
        }
    )
