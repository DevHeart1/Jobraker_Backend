...
class UserProfile(models.Model):
    ...
    experience_level = models.CharField(
        max_length=20, choices=EXPERIENCE_LEVELS, default="entry"
    )
    ...
    preferred_locations = models.JSONField(
        default=list, help_text="Preferred job locations"
    )
    ...
    if PGVECTOR_AVAILABLE:
        ...
        profile_embedding = models.TextField(
            null=True, blank=True, help_text="Vector embedding (JSON)"
        )
        skills_embedding = models.TextField(
            null=True, blank=True, help_text="Vector embedding (JSON)"
        )
