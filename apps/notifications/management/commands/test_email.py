...
from apps.notifications.tasks import (process_daily_job_alerts,
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--user-email", type=str, help="Email address of test user")
    ...
    def handle(self, *args, **options):
        ...
        if test_type in ["recommendations", "all"]:
            ...
            success = email_service.send_job_recommendation_email(
                user=user, recommended_jobs=mock_recommendations
            )
            if success:
                self.stdout.write(
