...
from apps.notifications.health_checks import (
    check_email_service, check_celery, check_redis, 
    check_websocket, check_database, check_templates
)
...
def check_production_requirements():
    ...
    # Check SECRET_KEY
    if not getattr(settings, 'SECRET_KEY', None) or settings.SECRET_KEY == 'dev-secret-key-please-change-in-production':
    ...
    return {
        ...
        "status": "All production requirements met" if len(issues) == 0 else f"{len(issues)} issues found",
        "issues": issues
    }
    ...
    try:
        with connection.cursor() as cursor:
            ...
            return {
                ...
                "total_connections": connection.queries_limit if hasattr(connection, 'queries_limit') else "unknown"
            }
    except Exception as e:
        return {"error": str(e)}
