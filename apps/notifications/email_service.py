...
class EmailService:
    ...
    def __init__(self):
        self.from_email = getattr(
            settings, "DEFAULT_FROM_EMAIL", "noreply@jobraker.com"
        )
    ...
    def send_email(
        ...
        try:
            ...
            context.update(
                {
                    ...
                    "support_email": getattr(
                        settings, "SUPPORT_EMAIL", "support@jobraker.com"
                    ),
                }
            )
            ...
            logger.error(
                f"Failed to send email to {recipient_list}: {e}", exc_info=True
            )
            return False

    def send_job_alert_email(
        self, user: User, jobs: List[Job], alert: JobAlert
    ) -> bool:
        """
    ...
    def send_application_status_update(
        ...
        context = {
            ...
            "status_message": status_messages.get(
                application.status, "Your application status has been updated."
            ),
        ...
        subject = (
            f"Application Update: {application.job.title} at {application.job.company}"
        )
    ...
    def send_job_recommendation_email(
        self, user: User, recommended_jobs: List[Dict[str, Any]]
    ) -> bool:
        """
    ...
    def send_application_follow_up_reminder(self, application: Application) -> bool:
        ...
        context = {
            ...
            "days_since_application": (
                (application.created_at.date() - application.applied_at.date()).days
        ...
        subject = (
            f"Follow-up Reminder: {application.job.title} at {application.job.company}"
        )
