...
class Application(models.Model):
    ...
    STATUS_CHOICES = [
        ...
        (
            "failed_to_submit",
            "Failed to Submit",
        ),  # Generic submission failure (manual or other)
        (
            "skyvern_submission_failed",
            "Skyvern Submission Failed",
        ),  # Skyvern explicitly failed
        ("skyvern_canceled", "Skyvern Task Canceled"),  # Skyvern task was canceled
        (
            "skyvern_requires_attention",
            "Skyvern Task Requires Attention",
        ),  # Skyvern needs input/action
    ]
    ...
    application_type = models.CharField(
        max_length=20, choices=APPLICATION_TYPES, default="manual"
    )
    ...
    )  # New field for user-specific notes
    ...
    USER_DEFINED_STATUS_CHOICES = [
        ...
        (
            "offer_received_custom",
            "Offer Received (User Tracked)",
        ),  # To distinguish from system 'offer_received'
...
class SavedJob(models.Model):
    ...
    category = models.CharField(
        max_length=50, blank=True, help_text="User-defined category"
    )
...
class JobAlert(models.Model):
    ...
    experience_level = models.CharField(
        max_length=20, choices=Job.EXPERIENCE_LEVELS, blank=True
    )
...
class RecommendedJob(models.Model):
    ...
    STATUS_CHOICES = [
        ...
        (
            "applied",
            "Applied",
        ),  # User initiated an application from this recommendation
        ("dismissed", "Dismissed"),  # User indicated they are not interested
        (
            "irrelevant",
            "Marked as Irrelevant",
        ),  # User explicitly marked it as a bad match
    ]

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="recommended_jobs"
    )
    job = models.ForeignKey(
        Job, on_delete=models.CASCADE, related_name="recommendations_for_users"
    )
    ...
    class Meta:
        ...
        unique_together = (
            "user",
            "job",
        )  # A user should only have a specific job recommended once directly
