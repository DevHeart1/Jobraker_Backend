...
from .models import (Application, Job, JobAlert,  # Added RecommendedJob
                     JobSource, RecommendedJob, SavedJob)
from .serializers import \
    RecommendedJobSerializer  # Added RecommendedJobSerializer
from .serializers import (ApplicationSerializer, ApplicationStatsSerializer,
                          BulkApplySerializer, ErrorResponseSerializer,
                          JobAlertSerializer, JobListSerializer,
                          JobRecommendationSerializer,
                          JobSearchResultSerializer, JobSearchSerializer,
...
@extend_schema_view(
    list=extend_schema(
        ...
        responses={200: JobListSerializer(many=True), 401: ErrorResponseSerializer},
    ),
    retrieve=extend_schema(
        summary="Get job details",
        description="Retrieve detailed information about a specific job",
        tags=["Jobs"],
        responses={200: JobSerializer, 404: ErrorResponseSerializer},
    ),
...
class JobViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        responses={200: JobListSerializer(many=True), 404: ErrorResponseSerializer},
    )
    ...
    def similar(self, request, pk=None):
        ...
        serializer = JobListSerializer(
            similar_jobs, many=True, context={"request": request}
        )
                ...
                examples=[
                    OpenApiExample(
                        ...
                        value={
                            ...
                            "status": "queued",
                            "message": "Interview question generation has been queued.",
                        },
                    )
                ],
            ),
    ...
    def generate_interview_questions(self, request, pk=None):
        ...
        from apps.integrations.tasks import \
            generate_interview_questions_task  # Local import for clarity
        ...
        task = generate_interview_questions_task.delay(
            job_id=str(job.id), user_id=str(request.user.id)
        )
...
@extend_schema_view(
    list=extend_schema(
        ...
        tags=["Applications"],
        responses={200: ApplicationSerializer(many=True), 401: ErrorResponseSerializer},
    ),
    retrieve=extend_schema(
        summary="Get application details",
        description="Retrieve detailed information about a specific application",
        tags=["Applications"],
        responses={200: ApplicationSerializer, 404: ErrorResponseSerializer},
    ),
    create=extend_schema(
        ...
        responses={201: ApplicationSerializer, 400: ErrorResponseSerializer},
    ),
    update=extend_schema(
    ...
    def get(self, request):
        ...
        if valid_params.get("experience_level"):
            queryset = queryset.filter(
                experience_level=valid_params["experience_level"]
            )
        ...
        # Apply pagination
        page_size = min(
            int(request.query_params.get("limit", 20)), 50
        )  # Max 50 results
    ...
    def get_queryset(self):
        ...
        if status_param:
            ...
            if statuses_to_filter:
                ...
                logger.warning(
                    f"Invalid status values provided for recommendation filter: {statuses}"
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            if not hasattr(user, "profile"):
                logger.warning(
                    f"User {user.id} does not have a profile to generate recommendations."
                )
                return Response(
                    {
            ...
            recommendations_result = service.generate_recommendations_for_user(
            ...
            logger.error(
                f"An unexpected error occurred while generating recommendations for user {user.id}: {e}",
                exc_info=True,
            )
            return Response(
                {
        ...
        for job_id in job_ids:
            try:
                ...
                if Application.objects.filter(user=user, job=job).exists():
                    logger.warning(
                        f"User {user.id} attempting to auto-apply to already applied job {job.id}. Skipping."
                    )
                    continue
                ...
                logger.info(
                    f"Queued Skyvern application task {task.id} for user {user.id} and job {job.id}"
                )

            except Job.DoesNotExist:
                logger.warning(
                    f"Auto-apply requested for non-existent or inactive job_id: {job_id}"
                )
                continue
        ...
        for job_id in job_ids:
            try:
                ...
                if Application.objects.filter(user=request.user, job=job).exists():
                    failed_jobs.append(
                        {"job_id": job_id, "error": "Already applied to this job"}
                    )
                    continue
...
@extend_schema_view(
    list=extend_schema(
        ...
        tags=["Saved Jobs"],
        responses={200: SavedJobSerializer(many=True), 401: ErrorResponseSerializer},
    ),
    retrieve=extend_schema(
        summary="Get saved job details",
        description="Retrieve details of a specific saved job",
        tags=["Saved Jobs"],
        responses={200: SavedJobSerializer, 404: ErrorResponseSerializer},
    ),
    create=extend_schema(
        summary="Save a job",
        ...
        responses={201: SavedJobSerializer, 400: ErrorResponseSerializer},
    ),
    destroy=extend_schema(
...
@extend_schema_view(
    list=extend_schema(
        ...
        tags=["Job Alerts"],
        responses={200: JobAlertSerializer(many=True), 401: ErrorResponseSerializer},
    ),
    retrieve=extend_schema(
        summary="Get job alert details",
        description="Retrieve details of a specific job alert",
        tags=["Job Alerts"],
        responses={200: JobAlertSerializer, 404: ErrorResponseSerializer},
    ),
    create=extend_schema(
        ...
        responses={201: JobAlertSerializer, 400: ErrorResponseSerializer},
    ),
    update=extend_schema(
...
@extend_schema(
    ...
    tags=["Applications"],
    responses={200: ApplicationStatsSerializer, 401: ErrorResponseSerializer},
)
