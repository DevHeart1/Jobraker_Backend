...
class MockJob:
    objects = MagicMock()
    def __init__(self, id, combined_embedding=None, job_embedding=None, status='active', **kwargs):
        self.id = id
...
class TestJobMatchService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_user_profile = patch('apps.jobs.services.UserProfile', new_callable=lambda: MockUserProfile)
        ...
        self.patcher_vdb_service = patch('apps.jobs.services.VectorDBService', new_callable=lambda: MockVectorDBService)
        self.MockVDBServiceClass = self.patcher_vdb_service.start()

        from apps.jobs.services import JobMatchService # Import service after patching
    ...
    def test_find_matching_jobs_for_user_success(self):
        ...
        mock_profile = MockUserProfile(id=user_profile_id, profile_embedding=mock_embedding)
        self.MockUserProfileModel.objects.get.return_value = mock_profile

        mock_job_doc1_id = uuid.uuid4()
        mock_job_doc2_id = uuid.uuid4()
        mock_vdb_results = [
            {'source_id': str(mock_job_doc1_id), 'similarity_score': 0.9, 'text_content': 'job1 desc', 'source_type': 'job_listing'},
            {'source_id': str(mock_job_doc2_id), 'similarity_score': 0.8, 'text_content': 'job2 desc', 'source_type': 'job_listing'},
        ]
        self.job_match_service.vector_db_service.search_similar_documents.return_value = mock_vdb_results
        ...
        self.MockUserProfileModel.objects.get.assert_called_once_with(id=user_profile_id)
        ...
        self.MockJobModel.objects.filter.assert_called_once_with(id__in=[str(mock_job_doc1_id), str(mock_job_doc2_id)], status='active')

        self.assertEqual(len(results), 2)
        self.assertEqual(results[0]['job'], mock_job1) # Assuming order is preserved or re-sorted
    ...
    def test_find_matching_jobs_for_user_embedding_override(self):
        ...
        self.job_match_service.vector_db_service.search_similar_documents.return_value = [] # No need to check further

        self.job_match_service.find_matching_jobs_for_user(user_profile_id, user_profile_embedding_override=override_embedding)
    ...
    def test_find_matching_jobs_for_user_no_profile_embedding_uses_skills(self):
        ...
        mock_profile = MockUserProfile(id=user_profile_id, profile_embedding=None, skills_embedding=skills_embedding)
        self.MockUserProfileModel.objects.get.return_value = mock_profile
        ...
        self.job_match_service.vector_db_service.search_similar_documents.assert_called_once_with(
            query_embedding=skills_embedding, top_n=10, filter_criteria={'source_type': 'job_listing'}
        )

    def test_find_matching_jobs_for_user_no_embeddings_at_all(self):
        user_profile_id = uuid.uuid4()
        mock_profile = MockUserProfile(id=user_profile_id, profile_embedding=None, skills_embedding=None)
        self.MockUserProfileModel.objects.get.return_value = mock_profile
    ...
    def test_find_matching_jobs_for_user_profile_not_found(self):
        user_profile_id = uuid.uuid4()
        self.MockUserProfileModel.objects.get.side_effect = MockUserProfile.DoesNotExist # Mock DoesNotExist if it's custom or Django's
    ...
    def test_find_matching_jobs_no_vdb_results(self):
        ...
        mock_profile = MockUserProfile(id=user_profile_id, profile_embedding=mock_embedding)
        self.MockUserProfileModel.objects.get.return_value = mock_profile
        self.job_match_service.vector_db_service.search_similar_documents.return_value = [] # No docs found
    ...
    def test_score_job_for_user_success(self):
        ...
        with patch.object(self.job_match_service, '_calculate_cosine_similarity', return_value=0.85) as mock_calc:
    ...
    def test_score_job_for_user_embedding_overrides(self):
        ...
        with patch.object(self.job_match_service, '_calculate_cosine_similarity', return_value=0.7) as mock_calc:
            score = self.job_match_service.score_job_for_user(
                user_profile_id, job_id,
    ...
    def test_score_job_for_user_missing_user_embedding(self):
        ...
        mock_profile = MockUserProfile(id=user_profile_id, profile_embedding=None, skills_embedding=None)
        self.MockUserProfileModel.objects.get.return_value = mock_profile
        mock_job = MockJob(id=job_id, combined_embedding=[0.1]*1536) # Job has embedding
        self.MockJobModel.objects.get.return_value = mock_job
    ...
    def test_score_job_for_user_missing_job_embedding(self):
        user_profile_id = uuid.uuid4()
        job_id = uuid.uuid4()
        mock_profile = MockUserProfile(id=user_profile_id, profile_embedding=[0.1]*1536)
        self.MockUserProfileModel.objects.get.return_value = mock_profile
        mock_job = MockJob(id=job_id, combined_embedding=None, job_embedding=None) # Job has no embedding
        self.MockJobModel.objects.get.return_value = mock_job
    ...
    def test_calculate_cosine_similarity_invalid_input(self):
        self.assertIsNone(self.job_match_service._calculate_cosine_similarity(None, [1.0]))
        self.assertIsNone(self.job_match_service._calculate_cosine_similarity([1.0], None))
        self.assertIsNone(self.job_match_service._calculate_cosine_similarity([], []))
        self.assertIsNone(self.job_match_service._calculate_cosine_similarity([1.0, 2.0], [1.0]))
