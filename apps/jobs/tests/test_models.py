...
from apps.jobs.models import (  # Job is needed for JobTypeChoices if used directly
    Job, JobAlert)
...
class JobAlertModelTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="testuseralert@example.com", password="password123"
        )
    ...
    def test_job_alert_default_values(self):
        alert_with_defaults = JobAlert.objects.create(
            user=self.user, name="Defaults Test"
        )
        self.assertTrue(alert_with_defaults.is_active)
        self.assertEqual(alert_with_defaults.frequency, "daily")  # Default is 'daily'
        self.assertEqual(
            alert_with_defaults.keywords, []
        )  # Default for JSONField is list
...
class ApplicationModelAdvancedTrackingTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="testappuser@example.com", password="password"
        )
        self.job = Job.objects.create(
            title="Test Job for App",
            company="Test Co",
            description="Desc",
            location="Loc",
        )
    ...
    def test_application_user_defined_status_choices(self):
        self.application.user_defined_status = (
            "interviewing"  # Example valid choice from plan
        )
        ...
        self.application.user_defined_status = Application.USER_DEFINED_STATUS_CHOICES[
            4
        ][
            0
        ]  # 'initial_interview'
