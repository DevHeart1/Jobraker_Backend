...
from apps.jobs.models import (Application, Job, JobAlert,  # Added Application
                              RecommendedJob)
from apps.jobs.serializers import (  # Added ApplicationSerializer
    ApplicationSerializer, JobAlertSerializer)
...
class JobRecommendationsViewTests(APITestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create_user(
            email="user1@example.com", password="password1", first_name="User1"
        )
        ...
        self.user2 = User.objects.create_user(
            email="user2@example.com", password="password2", first_name="User2"
        )
        try:
        ...
        self.job1 = Job.objects.create(
            title="Dev Ops", company="Cloud Inc", description="desc1", location="Remote"
        )
        self.job2 = Job.objects.create(
            title="Frontend Dev", company="Web Co", description="desc2", location="NY"
        )
        self.job3 = Job.objects.create(
            title="Backend Dev", company="Data LLC", description="desc3", location="SF"
        )
        ...
        self.rec1_user1 = RecommendedJob.objects.create(
            user=self.user1, job=self.job1, score=0.9, status="pending_review"
        )
        self.rec2_user1 = RecommendedJob.objects.create(
            user=self.user1, job=self.job2, score=0.8, status="viewed"
        )
        self.rec3_user1 = RecommendedJob.objects.create(
            user=self.user1, job=self.job3, score=0.7, status="dismissed"
        )

        # Recommendation for user2
        self.rec1_user2 = RecommendedJob.objects.create(
            user=self.user2, job=self.job4, score=0.95, status="pending_review"
        )
    ...
    def test_get_recommendations_authenticated_user_default_status(self):
        ...
        self.assertEqual(
            len(results), 2
        )  # rec1_user1 (pending_review), rec2_user1 (viewed)
    ...
    def test_get_recommendations_no_recommendations_for_user(self):
        ...
        response = self.client.get(
            self.url
        )  # Default filters 'pending_review', 'viewed'
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        results = (
            response.data.get("results")
            if "results" in response.data
    ...
    def test_get_recommendations_ensure_user_sees_only_their_own(self):
        ...
        results_user1 = (
            response_user1.data.get("results")
            if "results" in response_user1.data
            else response_user1.data
        )
        ...
        results_user2 = (
            response_user2.data.get("results")
            if "results" in response_user2.data
            else response_user2.data
        )
...
class JobAlertViewSetTest(APITestCase):
    def setUp(self):
        self.user1 = User.objects.create_user(
            email="user1alert@example.com", password="password1"
        )
        self.user2 = User.objects.create_user(
            email="user2alert@example.com", password="password2"
        )
        ...
        self.list_create_url = reverse(
            "jobalert-list"
        )  # DRF default router name for list/create
    ...
    def test_create_job_alert_authenticated(self):
        ...
        self.assertEqual(
            new_alert.keywords, ["JavaScript", "React"]
        )  # Check JSONField data
    ...
    def test_partial_update_job_alert_owner_patch(self):
        ...
        detail_url = reverse("jobalert-detail", kwargs={"pk": self.alert1_user1.pk})
        patch_data = {
            "name": "User1 Python Alert Patched",
            "is_active": False,
            "location": "Anywhere",
        }
    ...
    def test_delete_job_alert_owner(self):
        ...
        self.assertEqual(
            JobAlert.objects.filter(user=self.user1).count(), initial_count - 1
        )
...
from apps.jobs.models import (  # Already imported Job, add Application if not already for this new class
    Application, Job)
from apps.jobs.serializers import \
    ApplicationSerializer  # Import ApplicationSerializer for this new class


class ApplicationViewSetAdvancedTrackingTest(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="apptrackuser@example.com", password="password"
        )
        self.job = Job.objects.create(
            title="Test Job for App Tracking",
            company="Track Co",
            description="Desc",
            location="Loc",
        )
        self.application = Application.objects.create(
            user=self.user, job=self.job, status="submitted"  # Initial system status
        )
        self.client = (
            APIClient()
        )  # Already in other test class, but good to have instance here
        self.client.force_authenticate(user=self.user)
        self.detail_url = reverse(
            "application-detail", kwargs={"pk": self.application.pk}
        )

    def test_update_application_with_new_tracking_fields(self):
        patch_data = {
            "user_defined_status": Application.USER_DEFINED_STATUS_CHOICES[1][
                0
        ...
        self.assertEqual(
            self.application.user_defined_status, patch_data["user_defined_status"]
        )
        ...
        self.assertEqual(
            self.application.interview_details, patch_data["interview_details"]
        )
    ...
    def test_cannot_update_follow_up_reminder_sent_at_via_api(self):
        ...
        original_sent_at = (
            self.application.follow_up_reminder_sent_at
        )  # Should be None initially

        patch_data = {"follow_up_reminder_sent_at": timezone.now()}
        response = self.client.patch(self.detail_url, patch_data, format="json")
        self.assertEqual(
        ...
        if original_sent_at:
            self.assertEqual(
                response.data["follow_up_reminder_sent_at"],
                original_sent_at.isoformat(),
            )
        else:
...
class JobViewSetInterviewQuestionActionTest(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="interview@example.com", password="password"
        )
        self.job = Job.objects.create(
        ...
        self.client.force_authenticate(user=self.user)
        self.url = reverse(
            "job-generate-interview-questions", kwargs={"pk": self.job.pk}
        )
    ...
    def test_generate_interview_questions_action_success(self, mock_task_delay):
        ...
        self.assertIn(
            "Interview question generation has been queued", response.data["message"]
        )

        mock_task_delay.assert_called_once_with(
            job_id=str(self.job.pk), user_id=str(self.user.pk)
        )
    ...
    def test_generate_interview_questions_action_job_not_found(self):
        ...
        invalid_url = reverse(
            "job-generate-interview-questions", kwargs={"pk": non_existent_uuid}
        )
        response = self.client.post(invalid_url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    @patch("apps.jobs.views.generate_interview_questions_task.delay")
    def test_generate_interview_questions_action_task_dispatch_failure(
        ...
        with self.assertLogs(
            logger="apps.jobs.views", level="ERROR"
        ) as cm:  # Assuming view logs errors
            response = self.client.post(self.url)
...
class JobSearchViewElasticsearchTest(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="essearch@example.com", password="password"
        )
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
        self.search_url = reverse(
            "job_search"
        )  # Assuming 'job_search' is the name of JobSearchView URL
    ...
    def test_job_search_view_uses_elasticsearch(self, mock_es_search):
        ...
        mock_search_instance.__getitem__.return_value = (
            mock_search_instance  # For slicing [0:50]
        )
    ...
    def test_job_search_view_handles_es_exception_gracefully(self, mock_es_search):
        mock_search_instance = MagicMock()
        mock_search_instance.execute.side_effect = Exception(
        ...
        self.assertEqual(
            response.data["error"], "Search service temporarily unavailable."
        )
    ...
    def test_job_search_view_query_construction(self, mock_es_search):
        ...
        mock_search_instance.__getitem__.return_value = (
            mock_search_instance  # For slicing
        )
