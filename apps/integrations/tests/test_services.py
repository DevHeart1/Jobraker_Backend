...
class OpenAIClientInterviewQuestionsTest(TestCase):
    ...
    def setUp(self):
        ...
        if not settings.configured:
            settings.configure(OPENAI_API_KEY="test_api_key_for_client") # Ensure API key is set for _validate_api_key
    ...
    def test_generate_interview_questions_success_json_parsing(self, mock_openai_create):
        job_title = "Software Engineer"
        job_description = "Develop and maintain web applications."
        expected_questions = [
            {"type": "Technical", "question": "Explain SOLID principles."},
            {"type": "Behavioral", "question": "Describe a time you overcame a technical challenge."}
        ]

        mock_choice = MagicMock()
        mock_choice.message.content = json.dumps(expected_questions) # Perfect JSON response
        ...
        self.assertIn("Return your response as a JSON list of objects", call_args['system_prompt'])
        self.assertEqual(call_args['messages'][0]['role'], 'user')

    @patch('apps.integrations.services.openai.openai.chat.completions.create')
    def test_generate_interview_questions_with_leading_trailing_text_json_parsing(self, mock_openai_create):
        job_title = "QA Analyst"
        job_description = "Ensure software quality."
        json_questions_str = json.dumps([{"type": "Situational", "question": "How do you handle bugs found late in cycle?"}])
        ...
        self.assertEqual(questions[0]['question'], "How do you handle bugs found late in cycle?")

    @patch('apps.integrations.services.openai.openai.chat.completions.create')
    def test_generate_interview_questions_invalid_json_structure(self, mock_openai_create):
        job_title = "Product Manager"
        job_description = "Define product vision."
        # JSON is valid, but not a list of {"type": ..., "question": ...}
        invalid_json_questions_str = json.dumps({"error": "bad format", "data": "some question here"})
    ...
    @patch('apps.integrations.services.openai.openai.chat.completions.create')
    def test_generate_interview_questions_empty_response_string(self, mock_openai_create):
        job_title = "Analyst"
    ...
    def test_generate_interview_questions_no_api_key(self):
        ...
        with self.assertRaises(ValueError) as context: # _validate_api_key raises ValueError
    ...
    def test_prompt_includes_user_profile_if_provided(self, mock_openai_create):
        ...
        mock_choice = MagicMock()
        mock_choice.message.content = json.dumps([{"type": "Technical", "question": "Explain Django ORM."}])
        mock_response = MagicMock()
        mock_response.choices = [mock_choice]
        mock_openai_create.return_value = mock_response
