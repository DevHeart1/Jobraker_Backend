...
class MockApplicationManager:
    def get(self, skyvern_task_id=None, id=None): # Allow get by id or skyvern_task_id
        ...
        raise Exception("MockApplicationManager.get not configured") # Should be DoesNotExist
...
@patch('apps.integrations.views.Application', new_callable=MagicMock) # Patch Application model in views
@patch('apps.integrations.views.logging.getLogger') # Patch logger
class TestSkyvernWebhookView(TestCase):

    def setUp(self):
        self.client = Client()
        self.webhook_url = reverse('webhook_receiver', kwargs={'service': 'skyvern'}) # Assuming URL name
    ...
    def _generate_signature(self, payload_body_bytes, secret):
        return hmac.new(secret.encode('utf-8'), payload_body_bytes, hashlib.sha256).hexdigest()

    @patch('apps.integrations.views.timezone.now')
    def test_webhook_valid_payload_completed(self, mock_timezone_now, mock_logger_getLogger, MockApplicationCls):
        ...
        self.assertEqual(mock_app_instance.skyvern_response_data, {"confirmation_id": "conf_abc", "submitted_on": "2023-01-01"})
    ...
    def test_webhook_invalid_signature(self, mock_logger_getLogger, MockApplicationCls):
        ...
        response = self.client.post(self.webhook_url, data=payload_bytes, content_type="application/json", HTTP_X_SKYVERN_SIGNATURE="wrong_signature")
    ...
    def test_webhook_no_secret_configured_should_fail_verification(self, mock_logger_getLogger, MockApplicationCls):
        mock_logger = mock_logger_getLogger.return_value
        ...
        payload = {"task_id": "task1", "status": "COMPLETED"}
        payload_bytes = json.dumps(payload).encode('utf-8')
        response = self.client.post(self.webhook_url, data=payload_bytes, content_type="application/json", HTTP_X_SKYVERN_SIGNATURE="any_sig")
        self.assertEqual(response.status_code, 403) # Should be forbidden as verification fails without secret
    ...
    def test_webhook_malformed_json(self, mock_logger_getLogger, MockApplicationCls):
        ...
        response = self.client.post(self.webhook_url, data=malformed_json_bytes, content_type="application/json", HTTP_X_SKYVERN_SIGNATURE=signature)
    ...
    def test_webhook_missing_task_id(self, mock_logger_getLogger, MockApplicationCls):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn("missing task_id or status", response.json().get("error", "").lower())

    def test_webhook_application_not_found(self, mock_logger_getLogger, MockApplicationCls):
        mock_logger = mock_logger_getLogger.return_value
        self.mock_app_manager._get_return_value = self.MockApplicationModel.DoesNotExist("Not found") # Raise DoesNotExist
        ...
        self.assertEqual(response.status_code, 200) # As per current logic, acknowledge even if app not found
        self.assertIn("webhook received for unknown task_id", response.json().get("message", "").lower())

    def test_webhook_status_failed_updates_application(self, mock_logger_getLogger, MockApplicationCls):
        mock_logger = mock_logger_getLogger.return_value
        ...
        payload = {"task_id": task_id, "status": "FAILED", "error_details": error_payload}
        payload_bytes = json.dumps(payload).encode('utf-8')
        signature = self._generate_signature(payload_bytes, "testsecret")
    ...
    def test_webhook_multiple_applications_returned(self, mock_logger_getLogger, MockApplicationCls):
        ...
        self.MockApplicationModel.MultipleObjectsReturned = type('MultipleObjectsReturned', (Exception,), {})
        self.mock_app_manager._get_return_value = self.MockApplicationModel.MultipleObjectsReturned("Multiple found")
        ...
        self.assertIn("multiple applications for task_id", response.json().get("error", "").lower())
