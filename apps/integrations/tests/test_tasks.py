...
class TestOpenAIRAGTasks(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_embedding_service = patch('apps.integrations.tasks.EmbeddingService', new_callable=lambda: MockEmbeddingService)
        self.MockEmbeddingServiceClass = self.patcher_embedding_service.start()

        self.patcher_vdb_service = patch('apps.integrations.tasks.VectorDBService', new_callable=lambda: MockVectorDBService)
        ...
        self.patcher_openai_calls_total = patch('apps.integrations.tasks.OPENAI_API_CALLS_TOTAL')
        self.mock_openai_calls_total = self.patcher_openai_calls_total.start()

        self.patcher_openai_call_duration = patch('apps.integrations.tasks.OPENAI_API_CALL_DURATION_SECONDS')
        self.mock_openai_call_duration = self.patcher_openai_call_duration.start()

        self.patcher_openai_moderation_checks = patch('apps.integrations.tasks.OPENAI_MODERATION_CHECKS_TOTAL')
        self.mock_openai_moderation_checks = self.patcher_openai_moderation_checks.start()

        self.patcher_openai_moderation_flagged = patch('apps.integrations.tasks.OPENAI_MODERATION_FLAGGED_TOTAL')
        self.mock_openai_moderation_flagged = self.patcher_openai_moderation_flagged.start()
        ...
        importlib.reload(tasks_module) # Reload to apply patches to module-level imports
    ...
    def test_get_openai_job_advice_task_with_rag_success(self):
        ...
        mock_user_profile_data = {"experience_level": "Mid", "skills": ["python", "django"]}

        # Mock EmbeddingService
        mock_query_embedding = [0.1] * 1536 # Standardized dimension from previous model
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.return_value = [mock_query_embedding]

        # Mock VectorDBService
        mock_rag_doc1_text = "Tip 1 for fintech resumes: highlight quantitative achievements."
        mock_rag_doc2_text = "Tip 2: mention specific fintech regulations if relevant."
        mock_vdb_results = [
            {'text_content': mock_rag_doc1_text, 'source_type': 'career_article', 'source_id': 'article1', 'similarity_score': 0.9},
            {'text_content': mock_rag_doc2_text, 'source_type': 'career_article', 'source_id': 'article2', 'similarity_score': 0.8},
        ]
        self.MockVDBServiceClass.return_value.search_similar_documents.return_value = mock_vdb_results
        ...
        result = integration_tasks.get_openai_job_advice_task(
            user_id, advice_type, context_query,
        ...
        self.assertEqual(result['model_used'], 'gpt-test-model') # From mock_django_settings

        # Verify EmbeddingService call
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.assert_called_once_with([context_query])
        ...
        self.assertIn(mock_rag_doc1_text, sent_messages[1]['content']) # User content prompt
        self.assertIn(mock_rag_doc2_text, sent_messages[1]['content'])
        self.assertIn("--- Start of Retrieved Information ---", sent_messages[1]['content'])

        # Verify metrics
        self.mock_openai_calls_total.labels(type='advice', model='gpt-test-model', status='success').inc.assert_called_once()
        self.mock_openai_call_duration.labels(type='advice', model='gpt-test-model').observe.assert_called_once()
    ...
    def test_get_openai_job_advice_task_no_rag_docs_found(self):
        ...
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.return_value = [[0.1]*1536]
        self.MockVDBServiceClass.return_value.search_similar_documents.return_value = [] # No docs found
        ...
        result = integration_tasks.get_openai_job_advice_task(user_id, advice_type, context_query, query_for_rag=context_query)

        self.assertTrue(result['success'])
        args, kwargs = self.mock_openai_chat_completion_create.call_args
        sent_messages = kwargs['messages']
        # Ensure RAG context markers are NOT in the prompt if no docs found
        self.assertNotIn("--- Start of Retrieved Information ---", sent_messages[1]['content'])
    ...
    def test_get_openai_job_advice_task_rag_embedding_fails(self):
        ...
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.return_value = None # Embedding fails
    ...
    def test_get_openai_job_advice_task_openai_api_error(self):
        ...
        self.mock_openai_chat_completion_create.side_effect = Exception("OpenAI API Error")

        with self.assertRaises(Retry): # Celery task should retry
            integration_tasks.get_openai_job_advice_task(user_id, advice_type, context_query, query_for_rag=context_query)

        self.mock_openai_calls_total.labels(type='advice', model='gpt-test-model', status='error').inc.assert_called_once()
    ...
    def test_get_openai_chat_response_task_with_rag_and_moderation_success(self):
        ...
        mock_user_profile_data = {"experience_level": "Senior", "skills": ["python", "api_design"]}
        ...
        mock_vdb_results = [{'text_content': mock_rag_job_text, 'source_type': 'job_listing', 'similarity_score': 0.88}]
        self.MockVDBServiceClass.return_value.search_similar_documents.return_value = mock_vdb_results
        ...
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.assert_called_once_with([message])
        expected_rag_filter = {'source_type': 'job_listing'} # Based on "jobs" in message
        self.MockVDBServiceClass.return_value.search_similar_documents.assert_called_once_with(
            query_embedding=mock_query_embedding, top_n=3, filter_criteria=expected_rag_filter
        )
        ...
        # Check metrics
        self.mock_openai_calls_total.labels(type='chat', model='gpt-test-model', status='success').inc.assert_called_once()
        self.mock_openai_call_duration.labels(type='chat', model='gpt-test-model').observe.assert_called_once()
        self.mock_openai_moderation_checks.labels(target='user_input').inc.assert_called_once()
        self.mock_openai_moderation_checks.labels(target='ai_output').inc.assert_called_once()
        self.mock_openai_moderation_flagged.labels(target='user_input').inc.assert_not_called()
        self.mock_openai_moderation_flagged.labels(target='ai_output').inc.assert_not_called()

    def test_get_openai_chat_response_task_input_flagged(self):
        user_id = 1
        flagged_message = "This is a flagged message."

        self.mock_openai_moderation_create.return_value = MagicMock(results=[MagicMock(flagged=True)])

        result = integration_tasks.get_openai_chat_response_task(user_id, flagged_message)
        ...
        self.mock_openai_moderation_checks.labels(target='user_input').inc.assert_called_once()
        self.mock_openai_moderation_flagged.labels(target='user_input').inc.assert_called_once()
    ...
    def test_get_openai_chat_response_task_output_flagged(self):
        ...
        self.mock_openai_moderation_checks.labels(target='ai_output').inc.assert_called_once()
        self.mock_openai_moderation_flagged.labels(target='ai_output').inc.assert_called_once()

    def test_analyze_openai_resume_task_with_rag_success(self):
        resume_text = "My resume..."
        target_job_description = "Looking for a senior backend developer with Python and AWS."
        ...
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.return_value = [mock_job_query_embedding]

        # Mock VectorDBService for RAG results (resume advice articles)
        mock_resume_advice_text = "For senior roles, emphasize leadership and system design."
        mock_vdb_results = [{'text_content': mock_resume_advice_text, 'source_type': 'career_advice', 'similarity_score': 0.92}]
        self.MockVDBServiceClass.return_value.search_similar_documents.return_value = mock_vdb_results

        # Mock OpenAI ChatCompletion
        mock_ai_analysis = "This is your AI generated resume analysis, considering the advice found."
        self.mock_openai_chat_completion_create.return_value = MagicMock(
            choices=[MagicMock(message=MagicMock(content=mock_ai_analysis))]
        )

        result = integration_tasks.analyze_openai_resume_task(
            resume_text, target_job=target_job_description, user_profile_data=mock_user_profile_data
        )
        ...
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.assert_called_once_with([target_job_description])

        # Verify VectorDBService call
        expected_rag_filter = {'source_type__in': ['career_advice', 'faq_item', 'interview_tips'],
                               'metadata__category__icontains': 'resume'}
        self.MockVDBServiceClass.return_value.search_similar_documents.assert_called_once_with(
            query_embedding=mock_job_query_embedding, top_n=2, filter_criteria=expected_rag_filter
        )

        # Verify OpenAI call (check that prompt includes RAG context)
        args, kwargs = self.mock_openai_chat_completion_create.call_args
        sent_messages = kwargs['messages'] # This task sends a single user prompt
        self.assertIn(mock_resume_advice_text, sent_messages[1]['content']) # User content prompt
        self.assertIn("--- Start: Relevant Resume Writing Advice ---", sent_messages[1]['content'])

        # Verify metrics
        self.mock_openai_calls_total.labels(type='resume_analysis', model='gpt-test-model', status='success').inc.assert_called_once()
        self.mock_openai_call_duration.labels(type='resume_analysis', model='gpt-test-model').observe.assert_called_once()
    ...
    def test_analyze_openai_resume_task_no_target_job_no_rag(self):
        ...
        result = integration_tasks.analyze_openai_resume_task(resume_text, target_job="") # Empty target_job
        ...
        args, kwargs = self.mock_openai_chat_completion_create.call_args
        sent_messages = kwargs['messages']
        self.assertNotIn("--- Start: Relevant Resume Writing Advice ---", sent_messages[1]['content'])

    # Tests for RAG Content Ingestion Tasks
    @patch('apps.integrations.tasks.Job') # Patch where Job model is imported by the task
    def test_generate_job_embeddings_and_ingest_for_rag_success(self, MockJobModel):
        job_id = str(uuid.uuid4())
        mock_job_instance = MagicMock(spec=MockJob) # Use our existing MockJob for attributes
        ...
        self.MockEmbeddingServiceClass.return_value.generate_job_embeddings.return_value = mock_embeddings_dict
        ...
        self.MockEmbeddingServiceClass.return_value.generate_job_embeddings.assert_called_once_with(mock_job_instance)
        mock_job_instance.save.assert_called_once_with(update_fields=['title_embedding', 'combined_embedding'])

        mock_vdb_instance.delete_documents.assert_called_once_with(source_type='job_listing', source_id=job_id)
        ...
        self.mock_openai_calls_total.labels(type='embedding_job', model=ANY, status='success').inc.assert_called_once()

    @patch('apps.integrations.tasks.KnowledgeArticle') # Patch where KnowledgeArticle is imported
    def test_process_knowledge_article_for_rag_task_active_article_success(self, MockKnowledgeArticleModel):
        article_id = 1
        ...
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.return_value = [mock_article_embedding]
        ...
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.assert_called_once_with([expected_text_to_embed])

        mock_vdb_instance.delete_documents.assert_called_once_with(source_type=mock_article_instance.source_type, source_id=str(article_id))
        ...
        self.mock_openai_calls_total.labels(type=f'embedding_knowledge_{mock_article_instance.source_type}', model=ANY, status='success').inc.assert_called_once()

    @patch('apps.integrations.tasks.KnowledgeArticle')
    def test_process_knowledge_article_for_rag_task_inactive_article_deletes_from_rag(self, MockKnowledgeArticleModel):
        article_id = 2
        mock_article_instance = MagicMock()
        mock_article_instance.id = article_id
        ...
        MockKnowledgeArticleModel.objects.get.assert_called_once_with(id=article_id)
        self.MockEmbeddingServiceClass.return_value.generate_embeddings.assert_not_called()
        mock_vdb_instance.delete_documents.assert_called_once_with(source_type="faq", source_id=str(article_id))
