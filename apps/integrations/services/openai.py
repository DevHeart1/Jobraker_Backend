...
from apps.common.metrics import (OPENAI_API_CALL_DURATION_SECONDS,
...
class OpenAIClient:
    ...
    def generate_embedding(
        ...
        try:
            ...
            OPENAI_API_CALLS_TOTAL.labels(
                type="embedding", model=model, status="success"
            ).inc()
            OPENAI_API_CALL_DURATION_SECONDS.labels(
                type="embedding", model=model
            ...
            OPENAI_API_CALLS_TOTAL.labels(
                type="embedding", model=model, status="error"
            ).inc()
    ...
    def generate_embeddings_batch(
        ...
        try:
            ...
            # Record metrics
            OPENAI_API_CALLS_TOTAL.labels(
                type="embedding_batch", model=model, status="success"
            ).inc()
            OPENAI_API_CALL_DURATION_SECONDS.labels(
                type="embedding_batch", model=model
            ).observe(time.time() - start_time)
            ...
            OPENAI_API_CALLS_TOTAL.labels(
                type="embedding_batch", model=model, status="error"
            ).inc()
    ...
    def chat_completion(
        ...
        # Use mock service if API key not configured
        if self.use_mock:
            return self.mock_service.chat_completion(
        ...
        try:
            ...
            # Record metrics
            OPENAI_API_CALLS_TOTAL.labels(
                type="chat", model=model, status="success"
            ).inc()
            ...
            OPENAI_API_CALLS_TOTAL.labels(
                type="chat", model=model, status="error"
            ).inc()
    ...
    def generate_interview_questions(
        ...
        user_profile_summary: Optional[
            str
        ] = None,  # Optional for future personalization
    ) -> List[Dict[str, str]]:
        """
        ...
        if question_types:
            ...
            prompt_lines.append(
                f"Please include a mix of question types, focusing on: {types_str}."
            )
        else:
            prompt_lines.append(
                "Please include a mix of question types, such as behavioral, technical (if applicable to the role), situational, and questions to assess cultural fit."
            )
            ...
            if (
                ...
                try:
                    ...
                    if isinstance(questions, list) and all(
                        isinstance(q, dict) and "type" in q and "question" in q
                        for q in questions
                    ):
                        return questions
                    else:
                        logger.warning(
                            f"OpenAI response for interview questions was valid JSON but not in the expected format: {json_str}"
                        )
                        # Fallback: Try to parse as one question if it's a simple string response not matching JSON list
                        return (
                            [{"type": "general", "question": response_text.strip()}]
                    ...
                    logger.error(
                        f"Failed to decode JSON response from OpenAI for interview questions: {je}. Response was: {json_str}"
                    )
                ...
                logger.warning(
                    f"Could not find JSON list in OpenAI response for interview questions: {response_text}"
                )
                return (
                    [{"type": "general", "question": response_text.strip()}]
                    if len(response_text.strip()) > 10
...
class EmbeddingService:
    ...
    def generate_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            return self.client.generate_embeddings_batch(
    ...
    def generate_user_profile_embeddings(self, user_profile) -> Dict[str, List[float]]:
        ...
        try:
            ...
            skills_embedding = (
                self.client.generate_embedding(skills_text) if skills_text else []
            )
            ...
            logger.error(
                f"Error generating profile embeddings for user {user_profile.user.id}: {e}"
            )
            return {}
