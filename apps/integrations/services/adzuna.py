...
from apps.common.metrics import (ADZUNA_API_ERRORS_TOTAL,
...
class AdzunaAPIClient:
    ...
    CB_RESET_TIMEOUT_SECONDS = (
        60  # Time in seconds before trying again when circuit is OPEN
    )
    ...
    def __init__(self):
        ...
        if not self.app_id or not self.api_key:
            logger.warning(
                "Adzuna API credentials not configured. API calls will use mock data."
            )
    ...
    def _handle_circuit_breaker_open(self):
        ...
        if (
            ...
            ADZUNA_CIRCUIT_BREAKER_STATE_CHANGES_TOTAL.labels(
    ...
    def _handle_circuit_breaker_failure(self):
        ...
        if self._cb_state == STATE_HALF_OPEN:
            ...
            logger.warning(
                "Circuit Breaker: HALF_OPEN request failed. State changed back to OPEN."
            )
        elif (
            self._cb_failures >= self.CB_MAX_FAILURES and self._cb_state != STATE_OPEN
        ):  # Avoid double increment if already open
            self._cb_state = STATE_OPEN
            ADZUNA_CIRCUIT_BREAKER_STATE_CHANGES_TOTAL.labels(
                new_state=STATE_OPEN
            ).inc()
            logger.warning(
                f"Circuit Breaker: Max failures ({self.CB_MAX_FAILURES}) reached. State changed to OPEN."
            )
    ...
    def _handle_circuit_breaker_success(self):
        ...
        if self._cb_state == STATE_HALF_OPEN:
            ...
            if self._cb_half_open_success_count >= self.CB_HALF_OPEN_MAX_REQUESTS:
                ...
                self._cb_last_failure_time = None
                logger.info(
                    "Circuit Breaker: HALF_OPEN requests successful. State changed to CLOSED."
                )
    ...
    def _make_request(
        ...
        try:
            ...
            if e.response.status_code == 429:
                logger.warning(
                    f"Adzuna API rate limit hit (429): {e}. Retries configured."
                )
            ...
            "results": [
                {
                    ...
                    "created": (datetime.now() - timedelta(days=i)).strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    ),
    ...
    def search_jobs(
        ...
        endpoint = (
            f"/jobs/{where.lower()}/search/{page}"  # Standardize country to lowercase
        )

        logger.info(
            f"Searching Adzuna jobs: what='{what}', where='{where}', page={page}, params={params}"
        )
    ...
    def get_salary_data(
        self, job_title: str, location: str = "", country: str = "us"
    ) -> Dict[str, Any]:
...
class AdzunaJobProcessor:
        ...
        try:
            for page in range(1, max_pages + 1):
                ...
                for job_data in jobs:
                    country_label = (
                        where  # Assuming 'where' is the country for this context
                    )
                    try:
                        result = self._process_job(
                            job_data, country_label
                        )  # Pass country for metrics
                        stats["processed"] += 1
                        if result == "created":
                            stats["created"] += 1
                        elif result == "updated":
                        ...
                        logger.error(
                            f"Error processing job {job_data.get('id', 'unknown')}: {e}"
                        )
                        stats["errors"] += 1
                        ADZUNA_JOBS_PROCESSED_TOTAL.labels(
                            country=country_label, status="error"
                        ).inc()
    ...
    def sync_recent_jobs(
        self, days: int = 1, countries: Optional[List[str]] = None
    ) -> Dict[str, int]:
        """
        ...
        logger.info(
            f"Starting Adzuna recent jobs sync for {days} day(s) in countries: {countries}"
        )
        ...
        for country_code in countries:
            ...
            for category in general_categories:
                logger.info(
                    f"Fetching category '{category}' for country '{country_code}'"
                )
                try:
                    ...
                    logger.error(
                        f"Error syncing category '{category}' in '{country_code}': {e}"
                    )
                    overall_stats[
                        "errors"
                    ] += 1  # Count this as a general error for the category sync

        logger.info(f"Adzuna recent jobs sync completed: {overall_stats}")
        return overall_stats

    def _process_job(
        self, job_data: Dict[str, Any], country_label: str = "unknown"
    ) -> str:
        """
        ...
        if not external_id:
            ...
            ADZUNA_JOBS_PROCESSED_TOTAL.labels(
                country=country_label, status="skipped_no_id"
            ).inc()
            return "skipped"
        ...
        existing_job = Job.objects.filter(
            external_id=external_id, external_source="adzuna"
        ).first()
        ...
        if existing_job:
            ...
            existing_job.save()
            ADZUNA_JOBS_PROCESSED_TOTAL.labels(
                country=country_label, status="updated"
            ).inc()

            # Queue job for embedding generation if not already processed
            if not existing_job.processed_for_matching:
                from apps.integrations.tasks_enhanced import \
                    process_job_for_embeddings
            ...
            ADZUNA_JOBS_PROCESSED_TOTAL.labels(
                country=country_label, status="created"
            ).inc()
