...
class Command(BaseCommand):
    ...
    def run_quick_check(self, config_service, options):
        ...
        results = {
            'openai': {
                ...
                'api_key_length': len(config_service.openai_api_key) if config_service.openai_api_key else 0
            },
            'adzuna': {
                'configured': bool(config_service.adzuna_app_id and config_service.adzuna_api_key),
                'app_id_length': len(config_service.adzuna_app_id) if config_service.adzuna_app_id else 0,
                'api_key_length': len(config_service.adzuna_api_key) if config_service.adzuna_api_key else 0
            },
            'skyvern': {
                'configured': bool(config_service.skyvern_api_key),
                'api_key_length': len(config_service.skyvern_api_key) if config_service.skyvern_api_key else 0,
        ...
        for api_name, config in results.items():
            ...
            if api_name == 'openai':
                if config['configured']:
                    self.stdout.write(f'  API Key: {config["api_key_length"]} characters')
                else:
    ...
    def display_api_result(self, api_name, result):
        ...
        if result['status'] == 'success':
            self.stdout.write(self.style.SUCCESS(f'✓ {api_name.upper()}: {result["message"]}'))
            
            # Display additional info if available
            if 'model' in result:
                self.stdout.write(f'  Model: {result["model"]}')
            if 'embedding_dimension' in result:
                self.stdout.write(f'  Embedding dimension: {result["embedding_dimension"]}')
            if 'total_jobs_available' in result:
                self.stdout.write(f'  Jobs available: {result["total_jobs_available"]}')
        else:
            self.stdout.write(self.style.ERROR(f'✗ {api_name.upper()}: {result["message"]}'))
