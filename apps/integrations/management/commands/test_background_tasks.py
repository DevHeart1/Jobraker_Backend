...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Testing background tasks: {test_type}"))
    ...
    def test_job_processing(self, dry_run=False):
        ...
        try:
            from apps.integrations.tasks import (automated_daily_job_sync,
                                                 cleanup_old_jobs,
                                                 fetch_adzuna_jobs,
    ...
    def test_ai_processing(self, dry_run=False):
        ...
        try:
            from apps.integrations.tasks import (
                batch_generate_job_embeddings, batch_generate_user_embeddings,
                generate_job_embeddings_and_ingest_for_rag,
                generate_user_profile_embeddings, intelligent_job_matching)
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    f"Queued job embeddings task: {job_embeddings_result.id}"
                )
                self.stdout.write(
                    f"Queued user embeddings task: {user_embeddings_result.id}"
                )
    ...
    def test_notification_processing(self, dry_run=False):
        ...
        try:
            from apps.notifications.tasks import (
                process_daily_job_alerts, process_weekly_job_alerts,
                send_application_follow_up_reminders,
                send_weekly_job_recommendations, send_welcome_email_task)
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    "Would test: Welcome emails, job alerts, recommendations"
                )

            self.stdout.write(
                self.style.SUCCESS("✓ Notification processing tasks available")
            )
            ...
            self.stdout.write(
                self.style.ERROR(f"✗ Error testing notification tasks: {e}")
            )
    ...
    def test_skyvern_processing(self, dry_run=False):
        ...
        try:
            from apps.integrations.tasks import (
                monitor_skyvern_task, process_pending_applications,
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    f"Queued Skyvern processing task: {skyvern_result.id}"
                )
            ...
            self.stdout.write(
                self.style.SUCCESS("✓ Skyvern processing tasks available")
            )
    ...
    def test_health_checks(self):
        ...
        try:
            ...
            if stats:
                ...
                self.stdout.write(
                    "  Start workers with: celery -A jobraker worker --loglevel=info"
                )
            ...
            try:
                ...
                if result == "ok":
                    ...
                    self.stdout.write(
                        self.style.WARNING("⚠ Redis/Cache connectivity issues")
                    )
            except Exception as e:
            ...
            try:
                ...
                self.stdout.write(
                    f"✓ Database connectivity working ({user_count} users)"
                )
            except Exception as e:
            ...
            for module_name, class_name in services_to_test:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"✗ {class_name} import error: {e}")
                    )
                except Exception as e:
