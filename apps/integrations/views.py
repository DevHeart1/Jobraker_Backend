...
class WebhookView(APIView):
    ...
    def post(self, request, service=None):
        ...
        if service == "adzuna":
            ...
            return Response(
                {"error": "Unknown service"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def _verify_skyvern_webhook_signature(self, request):
        ...
        skyvern_signature_header = request.headers.get(
            "X-Skyvern-Signature"
        )  # Common header, confirm with Skyvern docs
        shared_secret = getattr(settings, "SKYVERN_WEBHOOK_SECRET", None)
        ...
        if not shared_secret:
            logger.error(
                "Skyvern webhook: SKYVERN_WEBHOOK_SECRET is not configured in settings. Cannot verify signature."
            )
        ...
        try:
            ...
            expected_signature = hmac.new(
                shared_secret.encode("utf-8"), payload_body, hashlib.sha256
            ).hexdigest()
            ...
            if hmac.compare_digest(expected_signature, skyvern_signature_header):
                ...
                logger.warning(
                    f"Skyvern webhook: Invalid signature. Header: {skyvern_signature_header}, Calculated: {expected_signature}"
                )
                return False
    ...
    def _handle_skyvern_webhook(self, request):
        ...
        if not self._verify_skyvern_webhook_signature(
            request
        ):  # Self is needed for instance method
            return Response(
        ...
        if not skyvern_task_id or not skyvern_status:
            logger.error(
                f"Skyvern webhook: Missing task_id or status in payload: {payload}"
            )
            return Response(
        ...
        try:
            ...
            logger.warning(
                f"Skyvern webhook: Update for unknown Skyvern task_id {skyvern_task_id}. No matching application."
            )
            return Response(
                {"message": "Webhook for unknown task_id received."},
                status=status.HTTP_200_OK,
            )
        except Application.MultipleObjectsReturned:
            logger.error(
                f"Skyvern webhook: Multiple applications for Skyvern task_id {skyvern_task_id}. Critical error."
            )
            return Response(
                {"error": "Internal error: multiple applications for task_id."},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        logger.info(
            f"Processing Skyvern webhook for AppID: {application.id}, SkyvernTaskID: {skyvern_task_id}, SkyvernStatus: {skyvern_status}"
        )
        ...
        if skyvern_status == "COMPLETED":
            ...
            current_response_data.update(
                skyvern_error_details
                or skyvern_data
                or {"error": "Task failed as per webhook."}
            )
            application.skyvern_response_data = current_response_data
            fields_to_update.append("skyvern_response_data")
            ...
            current_response_data.update(
                {
                    ...
                    "details": skyvern_data or skyvern_error_details,
                }
            )
            application.skyvern_response_data = current_response_data
            fields_to_update.append("skyvern_response_data")
            ...
            current_response_data.update(
                skyvern_data
                or skyvern_error_details
                or {"status_reason": "requires_attention"}
            )
            application.skyvern_response_data = current_response_data
            fields_to_update.append("skyvern_response_data")
            ...
            logger.warning(
                f"Skyvern webhook: Unhandled Skyvern status '{skyvern_status}' for task {skyvern_task_id}."
            )

        if (
            new_app_status != original_app_status
            or "applied_at" in fields_to_update
            or "skyvern_response_data" in fields_to_update
        ):
            ...
            logger.info(
                f"Application {application.id} updated via webhook. Status: {original_app_status} -> {new_app_status}."
            )
        else:
            logger.info(
                f"Application {application.id} status ({original_app_status}) effectively unchanged by Skyvern webhook status {skyvern_status}."
            )

        return Response(
            {
                "message": "Webhook processed.",
                "application_updated": response_changed_application,
            }
        )
...
@extend_schema(
    ...
    request=OpenApiExample(
        ...
        value={"source": "adzuna", "full_sync": False, "max_jobs": 1000},
    ),
    responses={
        200: OpenApiExample(
            "Sync Triggered",
        ...
        try:
            ...
            if source == "adzuna" or source == "all":
                ...
                task = fetch_adzuna_jobs.delay(
                    categories=None, max_days_old=max_days_old  # Fetch all categories
                )
            ...
            return Response(
                {"error": "Failed to trigger job sync", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    responses={
        200: OpenApiExample(
            ...
            value={
                ...
                "response_time": 245,
                "api_version": "1.2.3",
                "rate_limit": {"remaining": 950, "reset_at": "2025-06-16T09:00:00Z"},
            },
        ),
        400: OpenApiExample(
...
class IntegrationConfigView(APIView):
    ...
    @extend_schema(
        ...
        request=OpenApiExample(
            ...
            value={
                "adzuna": {"sync_frequency": "daily", "max_jobs_per_sync": 2000},
                "openai": {"temperature": 0.8, "max_tokens": 3000},
            },
        ),
        responses={
            200: OpenApiExample(
    ...
    def patch(self, request):
        ...
        return Response({"message": "Integration configuration update coming soon"})
